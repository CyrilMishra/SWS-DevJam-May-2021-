...
def converstion(request):
    ...
    if request.method == "POST" and request.POST.get('send'):
        ...
        if (ChatTransaction.objects.filter(s=ss) & ChatTransaction.objects.filter(d=sdid)) or (
                ChatTransaction.objects.filter(s=ds) & ChatTransaction.objects.filter(d=ids)):
            data = ((ChatTransaction.objects.filter(s=ss) & ChatTransaction.objects.filter(d=sdid)) | (
                        ChatTransaction.objects.filter(s=ds) & ChatTransaction.objects.filter(d=ids)))
            sdata = data[0]
            # print(sdata)
    ...
    ss = Signup.objects.get(student_id=ids)
    ds = Signup.objects.get(student_id=sdid)
    data_message = (Message.objects.filter(s=ss).order_by('time') & Message.objects.filter(d=sdid)).order_by('time') | (
                Message.objects.filter(s=ds).order_by('time') & Message.objects.filter(d=ids)).order_by('time')
...
def chatapp(request):
    ...
    d = ChatTransaction.objects.filter(s=ss).order_by('-time') | ChatTransaction.objects.filter(d=Myid).order_by(
        '-time')
